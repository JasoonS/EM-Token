...
//        assert.equal(tx.logs[0].args.expiration, , "Incorrect argument in HoldCreated event");
    ...
    it("A payout just ordered and the associated hold should be registered correctly", async () => {
    ...
    it("Cancelled payout request and the associated hold should be correctly reflected", async () => {
        ...
        assert.equal(await instance.doesHoldExist.call(userAccount2, PAYOUT_ID1), true, "Cancelled payout request does not exist");
    ...
    // Transaction #2, will be executed by the operator
...
//        assert.equal(tx.logs[0].args.expiration, , "Incorrect argument in HoldCreated event");
    ...
    it("(Again) Just ordered payout request and the associated hold should be correctly stored", async () => {
        ...
        assert.equal(await instance.doesHoldExist.call(userAccount1, PAYOUT_ID2), true, "Cancelled payout request does not exist");
    ...
    it("Payout request in process and associated hold should be correctly reflected", async () => {
    ...
    it("A payout just put in suspense and the associated hold should be registered correctly", async () => {
        assert.equal(await instance.doesPayoutExist.call(userAccount1, PAYOUT_ID2), true, "Cancelled payout request does not exist");
    ...
    it("Operator should not be able to process or reject a payout that has put funds in suspense, and user shoould not be able to cancel or reorder it", async () => {
        ...
        truffleAssert.reverts(instance.rejectPayout(userAccount1, PAYOUT_ID2, "No particular reason", {from:operator}), "", "Was able to reject payout");
        truffleAssert.reverts(instance.cancelPayout(PAYOUT_ID2, {from:userAccount1}), "", "Was able to cancel payout");
        await truffleAssert.reverts(instance.orderPayout(PAYOUT_ID2, 10, "No particular instructions", {from:userAccount1}), "", "Was able to re-order payout");
    });
    ...
    it("Executed payout and the associated hold should be correctly reflected", async () => {
        assert.equal(await instance.doesPayoutExist.call(userAccount1, PAYOUT_ID2), true, "Cancelled payout request does not exist");
    ...
    // Transaction #3, will be executed by the operator
    ...
    it("(Again 2) Just ordered payout request and associated hold should be correctly stored", async () => {
        ...
        assert.equal(await instance.doesHoldExist.call(userAccount2, PAYOUT_ID3), true, "Cancelled payout request does not exist");
    ...
    // Transaction #4, will be put InProcess and rejected by the operator
    ...
    it("(Again 3) Just ordered payout request and associated hold should be correctly stored", async () => {
        ...
        assert.equal(await instance.doesHoldExist.call(userAccount3, PAYOUT_ID4), true, "Hold associated to executed payout request does not exist");

        _result = await instance.retrieveHoldData.call(userAccount3, PAYOUT_ID4);
    ...
    it("Rejected payout request and associated hold should be correctly reflected", async () => {
    ...
    it("(Again 4) Just ordered payout request and associated hold should be correctly stored", async () => {
        ...
        assert.equal(await instance.doesHoldExist.call(userAccount2, PAYOUT_ID5), true, "Hold associated to executed payout request does not exist");

        _result = await instance.retrieveHoldData.call(userAccount2, PAYOUT_ID5);
        assert.equal(_result.from, userAccount2, "From not correctly registered");
        assert.equal(_result.to, SUSPENSE_WALLET, "To not correctly registered");
        assert.equal(_result.notary, ZERO_ADDRESS, "Notary not correctly registered");
        assert.equal(_result.amount, 1000, "Amount not correctly registered");
    ...
    it("Rejected payout request should be correctly reflected", async () => {
    ...
    it("Approved users should be able to request payout on behalf of others", async () => {
        tx = await instance.orderPayoutFrom(PAYOUT_ID1, userAccount3, 200000, "No particular instructions 6", {from:userAccount1})
    ...
    it("Just ordered payout request and associated hold should be correctly stored", async () => {
        assert.equal(await instance.doesPayoutExist.call(userAccount1, PAYOUT_ID1), true, "Submitted payout request does not exist");

        _result = await instance.retrievePayoutData.call(userAccount1, PAYOUT_ID1);
        assert.equal(_result.walletToDebit, userAccount3, "walletToDebit not correctly registered");
        ...
        assert.equal(await instance.doesHoldExist.call(userAccount1, PAYOUT_ID1), true, "Hold associated to executed payout request does not exist");
        
        _result = await instance.retrieveHoldData.call(userAccount1, PAYOUT_ID1);
        assert.equal(_result.from, userAccount3, "From not correctly registered");
        assert.equal(_result.to, SUSPENSE_WALLET, "To not correctly registered");
        assert.equal(_result.notary, ZERO_ADDRESS, "Notary not correctly registered");
        assert.equal(_result.amount, 200000, "Amount not correctly registered");
    ...
    it("No one other than orderer should be able to cancel a payout request", async () => {
        truffleAssert.reverts(instance.cancelPayout(PAYOUT_ID1, {from:userAccount3}), "", "Was able to cancel payout");
        truffleAssert.reverts(instance.cancelPayout(PAYOUT_ID1, {from:owner}), "", "Was able to cancel payout");
        await truffleAssert.reverts(instance.cancelPayout(PAYOUT_ID1, {from:operator}), "", "Was able to cancel payout");
    });

    it("Operator should be able to reject the payout", async () => {
        tx = await instance.rejectPayout(userAccount1, PAYOUT_ID1, "Because I can", {from:operator});

        assert.equal(tx.logs[0].event, "HoldReleased", "HoldExecuted event not issued");
        assert.equal(tx.logs[0].args.holder, userAccount1, "Incorrect argument in HoldExecuted event");
        assert.equal(tx.logs[0].args.operationId, PAYOUT_ID1, "Incorrect argument in HoldExecuted event");
        assert.equal(tx.logs[0].args.status, HoldStatusCode.ReleasedByNotary, "Incorrect argument in HoldExecuted event");

        assert.equal(tx.logs[1].event, "PayoutRejected", "PayoutRejected event not issued");
        assert.equal(tx.logs[1].args.orderer, userAccount1, "Incorrect argument in PayoutRejected event");
        assert.equal(tx.logs[1].args.operationId, PAYOUT_ID1, "Incorrect argument in PayoutRejected event");
        assert.equal(tx.logs[1].args.reason, "Because I can", "Incorrect argument in PayoutRejected event");
    });

    it("Rejected payout and associated hold should be correctly reflected", async () => {
        assert.equal(await instance.doesPayoutExist.call(userAccount1, PAYOUT_ID1), true, "Executed payout request does not exist");
    ...
    it("(Again) Approved users should be able to request payout on behalf of others", async () => {
        tx = await instance.orderPayoutFrom(PAYOUT_ID4, userAccount3, 200000, "No particular instructions 6", {from:userAccount1})
    ...
    it("Just ordered payout request and associated hold should be correctly stored", async () => {
        ...
        assert.equal(await instance.doesHoldExist.call(userAccount1, PAYOUT_ID4), true, "Hold associated to executed payout request does not exist");
        
        _result = await instance.retrieveHoldData.call(userAccount1, PAYOUT_ID4);
        assert.equal(_result.from, userAccount3, "From not correctly registered");
        assert.equal(_result.to, SUSPENSE_WALLET, "To not correctly registered");
        assert.equal(_result.notary, ZERO_ADDRESS, "Notary not correctly registered");
        assert.equal(_result.amount, 200000, "Amount not correctly registered");
    ...
    it("No one other than orderer should be able to cancel a payout request", async () => {
        truffleAssert.reverts(instance.cancelPayout(PAYOUT_ID4, {from:userAccount3}), "", "Was able to cancel payout");
        truffleAssert.reverts(instance.cancelPayout(PAYOUT_ID4, {from:owner}), "", "Was able to cancel payout");
        await truffleAssert.reverts(instance.cancelPayout(PAYOUT_ID4, {from:operator}), "", "Was able to cancel payout");
    });

    it("Orderer should be able to cancel the payout", async () => {
        tx = await instance.cancelPayout(PAYOUT_ID4, {from:userAccount1});

        assert.equal(tx.logs[0].event, "HoldReleased", "HoldExecuted event not issued");
        assert.equal(tx.logs[0].args.holder, userAccount1, "Incorrect argument in HoldExecuted event");
        assert.equal(tx.logs[0].args.operationId, PAYOUT_ID4, "Incorrect argument in HoldExecuted event");
        assert.equal(tx.logs[0].args.status, HoldStatusCode.ReleasedByNotary, "Incorrect argument in HoldExecuted event");

        assert.equal(tx.logs[1].event, "PayoutCancelled", "PayoutCancelled event not issued");
        assert.equal(tx.logs[1].args.orderer, userAccount1, "Incorrect argument in PayoutCancelled event");
        assert.equal(tx.logs[1].args.operationId, PAYOUT_ID4, "Incorrect argument in PayoutCancelled event");
    });

    it("Cancelled payout and associated hold should be correctly reflected", async () => {
        assert.equal(await instance.doesPayoutExist.call(userAccount1, PAYOUT_ID4), true, "Executed payout request does not exist");
    ...
    it("(Again 2) Approved users should be able to request payout on behalf of others", async () => {
